...
def set_alias_command():
    ...
    if waddle_helpers.command_exists(payload['alias']):
    ...
    if not waddle_helpers.command_exists(payload['command']):
    ...
    alias_command = waddle_helpers.get_alias_command(payload['alias'], community.id)
    if alias_command:
